...
if __name__ == '__main__':
    ...
    parser.add_argument('--resources_bucket', type=str)
    
    print('TRAINING_JOB_NAME:', os.environ['TRAINING_JOB_NAME'])
    ...
    passthrough_cols = [col for col in feature_columns_names if col not in one_hot_encode_cols and col not in drop_cols]

    preprocessor = ColumnTransformer(
            transformers=[
                ('one_hot', OneHotEncoder(handle_unknown='ignore'), one_hot_encode_cols),
                ('drop', 'drop', drop_cols)
            ],
            remainder='passthrough') 

    onehot_cols = ColumnTransformer(
            transformers=[
                ('one_hot', OneHotEncoder(handle_unknown='ignore'), one_hot_encode_cols)
            ],
            remainder='drop').fit(concat_data).get_feature_names()


    preprocessor.fit(concat_data)

    # persist column names
    onehot_cols.extend(passthrough_cols)
    cols_df = pd.DataFrame(onehot_cols)    
    cols_df.to_csv('column_headers.csv',index=False,header=False)
    
    response = s3_client.upload_file('column_headers.csv', args.resources_bucket, 'free_trial_model/snowflake-hbomax-staging/schema/column_headers.csv', ExtraArgs={"ServerSideEncryption": "aws:kms"})
...
def predict_fn(input_data, model):
    ...
    # drop headers from features set
